...
def fastest_mirror(main_site='http://fsl.fmrib.ox.ac.uk/fsldownloads/',
    ...
    try:
        response = urllib.request.urlopen(url=mirror_url)
    except urllib.error.HTTPError as e:
    ...
    if len(servers) > 1:
        for mirror in list(servers.keys()):
...
class FslInstall(object):
    ...
    for release in range(10,16):
        supported['darwin'].vendors['apple'].releases.append(release)
    unsupported['darwin'].vendors['apple'].releases.append(16)
    ...
    def __init__(self, fsldir, version, mirror, keep, shell):
        ...
        if self.computer.vendor in list(self.unsupported['linux'].vendors.keys()):
        ...
        if self.computer.vendor not in list(supported_os[self.computer.os].vendors.keys()):
        ...
        if self.computer.os not in list(supported_os.keys()) or self.computer.version.major not in supported_os[self.computer.os].vendors[self.computer.vendor].releases:
...
def installer(options, args):
    ...
    if options.tarf:
        ...
        if this_computer.vendor == 'apple':
            ...
            for app in list(results.values()):
