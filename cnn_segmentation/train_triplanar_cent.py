...
def extract_single_image(img_path, lab_path,
    ...
    cent_picker = cent_picker_class(lab, list(range(1, 135)))
    patches = patch_picker.pick(voxels)
    cents = cent_picker.pick(voxels)

    voxel_labels = lab[voxels[:, 0], voxels[:, 1], voxels[:, 2]]
    return patches, cents, voxel_labels
...
def extract_data(img_pathes, lab_pathes,
    ...
    partial_extract_image = partial(extract_single_image,
                                    ...
                                    cent_picker_class=cent_picker_class,
    ...
    # unwrap the (patches, cents, labels) tuples, and concatenate arrays
    all_patches, all_cents, all_labels = zip(*all_results)
    ...
    all_patches, all_cents, all_labels = \
            shuffle_all(all_patches, all_cents, all_labels)

    if K.image_dim_ordering() == 'tf':
        all_patches = reshape_to_tf(all_patches, dim=4)
    n_all_tr_voxels = n_voxels_tr * len(img_pathes)

    patch_tr = all_patches[:n_all_tr_voxels]
    cent_tr = all_cents[:n_all_tr_voxels]
    y_tr = all_labels[:n_all_tr_voxels]

    patch_va = all_patches[n_all_tr_voxels:]
    cent_va = all_cents[n_all_tr_voxels:]
    y_va = all_labels[n_all_tr_voxels:]

    # does not override
    if out_path:
        dataset = {}
        dataset['patch'] = all_patches
        dataset['cent'] = all_cents
        dataset['label'] = all_labels
        write_h5file(out_path, dataset)
    return patch_tr, cent_tr, y_tr, patch_va, cent_va, y_va
...
def build_network(model_path, logger_path, stats_path,
    ...
    Arguments:
        ...
        scales: list of int
            The scales used
        retrain: bool
            Whether to train the network from scratch
    ...
    net = CnnTriplanarMultisetCentroids(patch_size=patch_size,
...
def train_triplanar_cent(img_pathes, lab_pathes,
    ...
    t_start = time.time()
    patch_tr, cent_tr, y_tr, patch_va, cent_va, y_va = \
            extract_data(img_pathes, lab_pathes,
                         n_voxels_tr, n_voxels_va,
                         patch_size, scales, is_parallel)

    print("Feature extracted in {} seconds".format(time.time()-t_start))
    net = build_network(model_path, logger_path, stats_path,
                        n_classes, patch_size, scales,
                        pretrain,
                        optimizer=optimizer,
                        init=init)
    ...
    patch_tr, patch_mu, patch_sigma = zscore(patch_tr)
    cent_tr, cent_mu, cent_sigma = zscore(cent_tr)
    patch_va = zscore(patch_va, mu=patch_mu, sigma=patch_sigma)[0]
    cent_va = zscore(cent_va, mu=cent_mu, sigma=cent_sigma)[0]

    y_tr = np_utils.to_categorical(y_tr, net.out_size)
    y_va = np_utils.to_categorical(y_va, net.out_size)
    feat_tr = [patch_tr, cent_tr]
    feat_va = [patch_va, cent_va]
    mu = [patch_mu, cent_mu]
    sigma = [patch_sigma, cent_sigma]
    net.set_train_stats(mu, sigma, len(y_tr), len(y_va))
    print("Start to train the model")
    net.train(feat_tr, y_tr,
              feat_va, y_va,
...
if __name__ == '__main__':
    ...
    train_triplanar_cent(img_pathes, lab_pathes,
                         model_path, stats_path, logger_path,
                         n_classes, patch_size, scales,
                         n_voxels_tr, n_voxels_va,
